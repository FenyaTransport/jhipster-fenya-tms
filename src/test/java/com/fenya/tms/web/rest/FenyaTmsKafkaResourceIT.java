...
import com.fenya.tms.config.KafkaSseConsumer;
import com.fenya.tms.config.KafkaSseProducer;
...
class FenyaTmsKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/fenya-tms-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/fenya-tms-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/fenya-tms-kafka/unregister"));
                return;
            }
        }
